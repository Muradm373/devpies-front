...
import { getListOfUsersByPage, resetListOfPatients } from "../../actions/AdminActions";
import {
  Card,
  Button,
  CardBody,
...
class PatientsManagement extends Component {
  constructor(props) {
    super(props);
    this.state = {
      isLazyLoading: false,
  ...
  componentDidMount() {
    this.props.resetListOfPatients();
    this.lazyLoad();
  }
  ...
  lazyLoad() {
    if (this.props.adminHasMorePatients) {
      this.setState({ isLazyLoading: true });
      setTimeout(() => {
        this.props.getListOfUsersByPage(this.state.page);
        this.setState({
          isLazyLoading: false,
        ...
        <div className=" text-center mt-1">
          <div className="text-left content mx-auto">
            ...
            <p className="description text-center w-75 mx-auto mt-0">
              Here you will be able to edit patients list.
            </p>
            <Row>
              <Col md="12">
                    ...
                    <BottomScrollListener onBottom={this.lazyLoad}>
                      {(scrollRef) => {
                        return (
                          ...
                          >
                            <div>
                              {this.props.listOfAdminPatients.map((patient) => (
                                <a
                                  class="dropdown-item d-flex align-items-center px-auto"
                                  href="#"
                                  style={{ height: "100px" }}
                                  onClick={() =>
                                    this.setState({ selectedUser: patient })
                                  }
                                >
                                  <div class="mr-3">
                                    <img
...
const mapDispatchToProps = (dispatch) => {
  return {
    getListOfUsersByPage: (page) => {
      getListOfUsersByPage(dispatch, page);
    },
    resetListOfPatients: () => {
      resetListOfPatients(dispatch)
    }
  };
};

export default connect(mapStateToProps, mapDispatchToProps)(PatientsManagement);
