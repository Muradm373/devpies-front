...
import {
  getListOfRepresentativesByPage,
  resetListOfRepresentatives,
} from "../../actions/AdminActions";

class RepresentativesManagement extends Component {
  constructor(props) {
    super(props);
    this.state = {
      isLazyLoading: false,
  ...
  componentDidMount() {
    this.props.resetListOfRepresentatives();
    this.lazyLoad();
  }

  lazyLoad() {
    if (this.props.adminHasMoreRepresentatives) {
      this.setState({ isLazyLoading: true });
      setTimeout(() => {
        this.props.getListOfRepresentativesByPage(this.state.page);
        this.setState({
          isLazyLoading: false,
                      ...
                      <CardBody>
                        <BottomScrollListener onBottom={this.lazyLoad}>
                          {(scrollRef) => {
                            return (
                              ...
                              >
                                <div>
                                  {this.props.listOfAdminRepresentatives.map(
                                    (x) => (
                                      <a
                        ...
                        >
                          <Fab
                            ...
                            aria-label="edit"
                          >
                            <i className="fas fa-user-md fa-1x mr-3" />
                            Add representative
                          </Fab>
                        </div>
                      </CardBody>
...
const mapDispatchToProps = (dispatch) => {
  return {
    getListOfRepresentativesByPage: (page) => {
      getListOfRepresentativesByPage(dispatch, page);
    },
    resetListOfRepresentatives: () => {
      resetListOfRepresentatives(dispatch);
    },
  };
};
