...
import {getListOfDoctorsByPage, resetListOfDoctors} from "../../actions/RepresentativeActions";
import {
  Card,
  Button,
  CardBody,
...
class DoctorsManagement extends Component {
  ...
  lazyLoad() {
    if (this.props.adminHasMoreDoctors) {
      ...
      setTimeout(() => {
        this.props.getListOfDoctorsByPage(this.state.page);
        this.setState({
          isLazyLoading: false,
                  ...
                  <BottomScrollListener onBottom={this.lazyLoad}>
                      {(scrollRef) => {
                        return (
                          ...
                          >
                            <div>
                              {this.props.listOfAdminDoctors.map((doctor) => (
                                <a
                                  class="dropdown-item d-flex align-items-center px-auto"
                                  href="#"
                                  style={{height: "100px"}}
                                  onClick={() =>
                                    this.setState({ selectedUser: doctor })
                                  }
                                >
                                  <div class="mr-3">
...
const mapDispatchToProps = (dispatch) => {
  return {
    getListOfDoctorsByPage: (page) => {
      getListOfDoctorsByPage(dispatch, page)
    },
    resetListOfDoctors: (page) => {
      resetListOfDoctors(dispatch, page)
    },
  };
};

export default connect(mapStateToProps, mapDispatchToProps)(DoctorsManagement);